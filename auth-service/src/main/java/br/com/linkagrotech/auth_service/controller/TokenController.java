...
public class TokenController {
	...
	if (wrapper.refreshToken == null || wrapper.refreshToken.equals("")) {
	    return ResponseEntity.badRequest()
		    .body(new ExcecaoDTO("Erro na operação", "Não foi possível extratir o header Authorization"));
	}
	...
	try {
	    return restTemplate.postForEntity(issuerUri + "/protocol/openid-connect/token", entity, String.class);
    ...
    @Operation(summary = "Request Token", description = "Login ")
    @PostMapping("/token")
	...
	try {
	    return rt.postForEntity(issuerUri + "/protocol/openid-connect/token", entity, String.class);
	} catch (HttpClientErrorException e) {
